...
function get_lists($cid,$num){
  ...
  $map['status'] = 1;
  $map['display'] = 1;
  $lists = M('Document')->where($map)->order('id desc')->limit($num)->select();
  //  dump(M('Document')->getlastsql());die; 
  return $lists;
}
...
function get_article($id){
  return M('category_content')->where('id='.$id)->find();
}

// 获取所有文章
function get_all_article($cid,$num){
  $map['category_id'] = $cid;
  $map['model_id'] = 2;
  $map['display'] = 1;
  $lists = M('Document')->where($map)->order('description,id desc')->limit($num)->select();
    // dump(M('Document')->getlastsql());die; 
  return $lists;
}

// 获取所有产品
function get_all_product($num){
  $cid = array('between','8,12');
  $map['category_id'] = $cid;
  $map['status'] = 1;
  $map['display'] = 1;
  $lists = M('Document')->where($map)->order('id desc')->limit($num)->select();
 //var_dump(M('Document')->getlastsql());  die;
  return $lists;
}
